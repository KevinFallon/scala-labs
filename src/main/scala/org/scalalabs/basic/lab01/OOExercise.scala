...
// prefixing vars with val or var makes them visible outside of the class since they become fields
class Euro (val euro: Int, val cents: Int = 0) extends Currency(symbol = "EUR") with Ordered[Euro] {
  require(euro >= 0)
  require(cents >= 0)
  ...
  override def toString = {
    val toStringCents:String =
      if (cents == 0) "--"
      else "%02d".format(cents)
    symbol + ": " + euro + "," + toStringCents
  }
}

object Euro {
  def fromCents(cents: Int): Euro = {
    new Euro( (cents - (cents % 100)) / 100, cents % 100)
  }
  implicit class EuroInt(val int: Int) extends AnyVal {
    def *(euro: Euro) = euro * int
  }
...
class Dollar (dollars:Int, cents: Int) extends Currency(symbol = "USD") {
  val inCents: Int = dollars * 100 + cents
}